...
int main( int argc, char *argv[] ) {
	...
	for( int tries = 0; tries < NUMTRIES; tries++ ) {
		...
		#pragma omp parallel for default(none) shared(vs,ths,gs,hs,ds,maxPerformance, stderr) reduction(+:numHits)
		for( int n = 0; n < NUMTRIALS; n++ ) {
			...
			float x = vx*t;
			if( x <= g ) {
				if( DEBUG )	fprintf( stderr, "Ball doesn't even reach the cliff %f\n", t );
			}
			else {
				// see if the ball hits the vertical cliff face:
				t = g/vx;
				float y = t*(vy + (.5*(GRAVITY)*(t)));
				if( y <= h ) {
